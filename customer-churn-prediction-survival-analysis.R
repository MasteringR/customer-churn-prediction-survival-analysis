...
dummies <- model.matrix(~ gender + Partner + Dependents + PhoneService + MultipleLines + InternetService + OnlineSecurity + OnlineBackup + DeviceProtection + TechSupport + StreamingTV + StreamingMovies + Contract + PaperlessBilling + PaymentMethod - 1, data = data)
...
colnames(data_with_dummies) <- gsub(" ", "_", colnames(data_with_dummies))

# Rename churn to Churn_Yes and make it a dummy variable too
data_with_dummies <- rename(data_with_dummies, Churn_Yes = Churn)
data_with_dummies <- data_with_dummies %>% mutate(Churn_Yes = if_else(Churn_Yes == "Yes", 1, 0))

# Normalization of numerical variables ----

# Apply min-max scaling to numeric features
data_with_dummies$tenure <- (data_with_dummies$tenure - min(data_with_dummies$tenure)) / (max(data_with_dummies$tenure) - min(data_with_dummies$tenure))
data_with_dummies$MonthlyCharges <- (data_with_dummies$MonthlyCharges - min(data_with_dummies$MonthlyCharges)) / (max(data_with_dummies$MonthlyCharges) - min(data_with_dummies$MonthlyCharges))
data_with_dummies$TotalCharges <- (data_with_dummies$TotalCharges - min(data_with_dummies$TotalCharges)) / (max(data_with_dummies$TotalCharges) - min(data_with_dummies$TotalCharges))
...
cox_fit <- coxph(surv_obj ~ ContractOne_year + ContractTwo_year + MonthlyCharges + CountOfServices, data = training_data)
